...
if [ ! -x "$CPY_PREFIX/bin/python3" ]; then
  ...
  echo ">> building static CPython $CPY_VER into $CPY_PREFIX (cached for re-runs)"
  builddir=$(mktemp -d)
  tar -C "$builddir" -xf "$CPY_TARBALL"
  (cd "$builddir/Python-$CPY_VER" \
     && ./configure --prefix="$CPY_PREFIX" CFLAGS="$DEBUG_FLAGS -O2" >configure.log 2>&1 \
...
#    $DEBUG_FLAGS). The launcher statically embeds the private CPython and runs the
#    harness in-process; PYTHONPATH is baked into the env the binary inherits at run
#    time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris +
#    torrent_parser.
HARNESS="$SRC/mayhem/differential_fuzz.py"
PY_CFLAGS="$("$PYCONF" --embed --cflags)"
# -rdynamic: C extension modules (atheris' native.so) resolve libpython symbols from
# the main executable, so the statically-linked interpreter must export them.
PY_LDFLAGS="-rdynamic $("$PYCONF" --embed --ldflags)"
echo ">> compiling torrent_parser_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS $PY_CFLAGS -DHARNESS="\"$HARNESS\"" \
...
# once when the harness is given a file path (no fuzzing loop), which is exactly the
